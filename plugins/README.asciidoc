...
在插件里也可以使用lckclub里已经开发好的service以及mapper
====

插件的实现原理是使用spring的aop切面编程思想实现的，lckclub里提供了service包下几乎所有service方法的切入点
...
[TIP]
====
注意不要将lckclub给打到jar包里了，在引入信赖的时候 `scope` 请配置成 `provided`
====
...
1. 将lckclub主项目打包，命令 `mvn clean package`
2. 将打包好的lckclub.jar拷贝到你想放的目录下，比如 `/opt/lckclub/`
3. 在`/opt/lckclub/`目录下创建一个文件夹 `plugins`
4. 将插件打包好后的jar包拷贝到 `plugins` 目录中
5. 启动项目使用命令 `java -Dloader.path=./plugins -jar lckclub.jar`
...
3. 插件包名必须为 `com.lckclub.plugin` （主题插件除外）
4. 插件打包必须要使用jdk8编译
5. 附上插件说明，越详细越好

首先下载lckclub项目，将`pom.xml`里的下面代码注释掉
...
然后运行命令 `mvn clean install` 将lckclub安装到本地maven库，这样在插件里就可以通过下面信赖引入了

[source,xml]
----
<dependency>
    <groupId>com</groupId>
    <artifactId>lckclub</artifactId>
...
然后在lckclub项目里的pom.xml里引入插件

最后启动lckclub，即可测试插件功能了
